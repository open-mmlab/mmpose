...
class Labelme2coco_keypoints():
    ...
    def _get_keypoints(self, points, keypoints, num_keypoints,label):
        ...
        例如：
            "keypoints": [
                ...
                1,                  # 相当于 Z 值，如果是2D关键点 v = 0表示不可见,v = 1表示标记但不可见，v = 2表示标记且可见
                82.42582269256718,
                109.95672933232304,
                1,
                ...,
    ...
    def _image(self, obj, path):
        ...
        image['height'], image['width'] = obj['imageHeight'], obj['imageWidth']  # 获得图片的宽高
    ...
    def _annotation(self, bboxes_list, keypoints_list, json_path):
        ...
        for object in bboxes_list:
            annotation = {}
            keypoints = [0 for i in range(36)]  # 每个keypoint数组初始化为[0,..] len = 36 对应12个点(x,y,v)
            num_keypoints = 0
            ...
            annotation['area'] = 1.0
            annotation['segmentation'] = [np.asarray(bbox).flatten().tolist()]# 两个坐标点
            ...
            for keypoint in keypoints_list:
                ...
                # annotation['keypoints'], num_keypoints = self._get_keypoints(point[0], keypoints, num_keypoints,label)
                num_keypoints = self._get_keypoints(point[0], keypoints, num_keypoints, label)
    ...
    def to_coco(self, json_path_list):
        ...
        keypoints = {}
        keypoints['info'] = {'description': 'Air Dataset', 'version': 1.0, 'year': 2022}
...
def convert(path,target):
    ...
    parser.add_argument("--class_name", "--n", help="class name", type=str, default='airplane')
    parser.add_argument("--input", "--i", help="json file path (labelme)", type=str, default=path)
    parser.add_argument("--output", "--o", help="output file path (coco format)", type=str, default=path)
    parser.add_argument("--join_num", "--j", help="number of join", type=int, default=12)
    parser.add_argument("--ratio", "--r", help="train and test split ratio", type=float, default=0.25)
