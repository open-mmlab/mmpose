...
def _get_mouth_keypoint_ids(model_cfg: Config) -> Tuple[int, int]:
    ...
    Returns:
        int: left-part nmouth keypoint index.
        int: right-part nmouth keypoint index.
    """
    # try obtaining mouth point ids from dataset_info
    try:
        dataset_info = DatasetInfo(model_cfg.data.test.dataset_info)
        left_mouth_idx = dataset_info.keypoint_name2id.get('face-48', None)
        right_mouth_idx = dataset_info.keypoint_name2id.get('face-54', None)
    except AttributeError:
        left_mouth_idx = None
...
@NODES.register_module()
class LoveHeartNode(BaseFrameEffectNode):
    ...
    # def _cal_angle(self, p1, p2, p3, p4):

    def _cal_angle(self, kpts: np.ndarray, hand_indices: List[int],
                   finger_indices: List[int]) -> np.float64:

        p1 = kpts[hand_indices[finger_indices[3]]][:2]
        p2 = kpts[hand_indices[finger_indices[2]]][:2]

        p3 = kpts[hand_indices[finger_indices[1]]][:2]
        p4 = kpts[hand_indices[finger_indices[0]]][:2]
    ...
    def _check_heart(self, pred: Dict[str, np.ndarray],
        ...
        Returns:
            int: a number representing the type of gesture,
        ...
        for i in [0, 17, 19, 20, 21, 38, 40, 41]:
            if kpts[hand_indices[i]][2] < self.kpt_thr:
                return 0

        left_indices = [0, 17, 19, 20]
        left_angle = self._cal_angle(kpts, hand_indices, left_indices)

        right_indices = [21, 38, 40, 41]
        right_angle = self._cal_angle(kpts, hand_indices, right_indices)
    ...
    def _get_start_and_end_pos(self, heart_type: int,
        ...
        for pose_result in pose_results:
            ...
            for i in range(2):
                ...
                if id in self.heart_infos.copy():
                    ...
                    mouth_indices = _get_mouth_keypoint_ids(model_cfg)

                    heart_type = self._check_heart(preds[i], hand_indices,
                                                   mouth_indices)
                    if heart_type:
                        start_pos, end_pos = self._get_start_and_end_pos(
