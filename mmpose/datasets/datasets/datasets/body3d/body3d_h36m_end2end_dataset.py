...
class Body3DH36MDataset_E2E(Body3DBaseDataset):
    ...
    def load_config(self, data_cfg):
        ...
        ann_info['image_size'] = np.array(data_cfg['image_size'])
        ann_info['heatmap_size'] = np.array(data_cfg['heatmap_size'])
        ann_info['heatmap3d_depth_bound'] = data_cfg['heatmap3d_depth_bound']
    ...
    def load_annotations(self):
        ...
        if not self.test_mode:
            data_info['imgnames'] = data_info['imgnames'][0:30000]
            data_info['joints_3d'] = data_info['joints_3d'][0:30000]
            data_info['joints_2d'] = data_info['joints_2d'][0:30000]
            data_info['scales'] = data_info['scales'][0:30000]
            data_info['centers'] = data_info['centers'][0:30000]
    ...
    def prepare_data(self, idx):
        ...
        results['rotation'] = 0
        results['joints_3d'] = np.pad(results['input_2d'][0], ((0, 0), (0, 1)),
                                      'constant')
        results['joints_3d_visible'] = np.tile(results['input_2d_visible'][0],
                                               3)
        return results
    ...
    def _report_mpjpe(self, keypoint_results, mode='mpjpe'):
        ...
        print('x_loss')
        print(np.linalg.norm(preds[:, :, 0] - gts[:, :, 0]))
        print('y_loss')
        print(np.linalg.norm(preds[:, :, 1] - gts[:, :, 1]))
        print('z_loss')
        print(np.linalg.norm(preds[:, :, 2] - gts[:, :, 2]))
