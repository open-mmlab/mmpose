...
def _get_multi_scale_size_udp(image, input_size, current_scale, min_scale):
    """Get the size for multi-scale training.
    ...
    size_resized = (w_resized, h_resized)
    scale = (scale_w, scale_h)
    return size_resized, center, scale
...
def _resize_align_multi_scale_udp(image, input_size, current_scale, min_scale):
    ...
    Returns:
        ...
        - scale (np.ndarray): scale
    """
    size_resized, _, _ = _get_multi_scale_size_udp(
        image, input_size, current_scale, min_scale)
...
@PIPELINES.register_module()
class BottomUpRandomAffine_udp:
    """Data augmentation with random scaling & rotating.
...
@PIPELINES.register_module()
class BottomUpGenerateTarget_udp:
    ...
    def _generate(self, num_joints, heatmap_size):
        ...
        heatmap_generator = [
            HeatmapGenerator_udp(output_size, num_joints, self.sigma)
    ...
    def __call__(self, results):
        ...
        target_list = list()
        results['joints_ori'] = results['joints'].copy()
...
class BottomUpResizeAlign_udp:
    ...
    def __call__(self, results):
        ...
        for _, s in enumerate(sorted(test_scale_factor, reverse=True)):
            _results = results.copy()
            image_resized, _, _ = _resize_align_multi_scale_udp(
                _results['img'], input_size, s, min(test_scale_factor))
            _results['img'] = image_resized
            _results = self.transforms(_results)
            transformed_img = _results['img'].unsqueeze(0)
