...
def camera_to_image_coord(root_index, kpts_3d_cam, camera_param):
    ...
    tl2d = camera_to_pixel(tl_kpt, fx, fy, cx, cy).flatten()
    br2d = camera_to_pixel(br_kpt, fx, fy, cx, cy).flatten()
    ...
    kpts_3d_depth = ratio * (kpts_3d_cam[:, 2] - kpts_3d_cam[root_index, 2])
    kpts_3d_image[:, 2] = kpts_3d_depth
    return kpts_3d_image, ratio
