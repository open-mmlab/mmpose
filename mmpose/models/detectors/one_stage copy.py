...
class OneStage(BasePose):
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        # NMS
        grouped, scores = pose_nms(self.test_cfg, img_metas, aggregated_heatmaps.cpu(), grouped)
        ...
        if self.test_cfg.match_hmp:
            grouped = match_pose_to_heatmap(self.test_cfg, grouped, aggregated_heatmaps)
        
        grouped = [torch.cat([torch.tensor(grouped[0]), torch.ones_like(torch.tensor(grouped[0][..., 0]).unsqueeze(-1))], dim=-1).numpy()]
        # recover size
        ...
        result['scores'] = scores  # [score1, score2, ..., scoreN]
