...
class VoxelPose(BasePose):
    ...
    def __init__(self,
        ...
        Args:
            detector_2d (dict): config for 2d detector
            space_3d (dict): space information
            project_layer (dict): config for project layer (3d to 2d)
            center_net (dict): config for center detection network
            center_head (dict): config for center detection head
            pose_net (dict): config for pose regression network
            pose_head (dict): config for pose regression head
            train_cfg (dict): Config for training. Default: None.
            test_cfg (dict): Config for testing. Default: None.
            pretrained (str): Path to the pretrained 2d detector model.
            freeze_2d (bool): whether to freeze the parameters of the
                        2d model. Default: True.
        """
