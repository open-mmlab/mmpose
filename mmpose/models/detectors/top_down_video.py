...
class TopDownVideo(BasePose):
    ...
    def forward_train(self, img, target, target_weight, img_metas, **kwargs):
        ...
        for i in range(num_frames):
            features = self.backbone(img[i])
            all_features.append(features)

        if self.with_neck:
            output = self.neck(all_features)
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        if self.with_neck:
            features = self.neck(all_features)
        ...
        if self.test_cfg.get('flip_test', True):
            ...
            all_features_flipped = []
            for i in range(num_frames):
                features_flipped = self.backbone(img_flipped[i])
                all_features_flipped.append(features_flipped)

            if self.with_neck:
                features_flipped = self.neck(all_features_flipped)
    ...
    def forward_dummy(self, img):
        ...
        output = [output for _ in range(3)]
        if self.with_neck:
