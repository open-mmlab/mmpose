...
我们推荐用户按照我们的最佳实践来安装 MMPose。但除此之外，如果你想根据
...
你可以在 `vis_results` 这个目录下看到输出的图片，这些图片展示了人体姿态估计的结果。
...
MMPose 可以仅在 CPU 环境中安装，在 CPU 模式下，你可以完成训练（需要 MMCV 版本 >= 1.4.4）、测试和模型推理等所有操作。
