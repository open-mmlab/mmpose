...
def main():
    ...
    # frame index offsets for inference
    indices = pose_model.cfg.data.test.data_cfg['frame_indices_test']

    print('Running inference...')
    for frame_id, frame in enumerate(mmcv.track_iter_progress(video)):
        frames = []
        ...
        # inference current frame with extra frames and a list of bboxes.
        pose_results, returned_outputs = inference_top_down_video_pose_model(
            pose_model,
            frames,
            person_results,
        ...
        if args.show:
            cv2.imshow('Current Frame', vis_frame)
