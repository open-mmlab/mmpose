    ...
    try:
        ...
        def track_by_iou(bbox, results_last, thr):
            ...
            for index, res_last in enumerate(results_last):
                ...
                # 计算IOU
                try:
                    iou_score = compute_iou(bbox, last_bbox)
        ...
        def compute_iou(bbox1, bbox2):
            """计算两个边界框的IOU"""
            try:
                # 确保边界框是标准格式 [x1, y1, x2, y2]
                if isinstance(bbox1, np.ndarray) and bbox1.size == 4:
                    bbox1 = bbox1.tolist()
                if isinstance(bbox2, np.ndarray) and bbox2.size == 4:
                    bbox2 = bbox2.tolist()
                ...
                if len(bbox1) >= 4 and len(bbox2) >= 4:
                    ...
                    # 计算两个框的面积
                    bbox1_area = (float(bbox1[2]) - float(bbox1[0])) * (float(bbox1[3]) - float(bbox1[1]))
                    bbox2_area = (float(bbox2[2]) - float(bbox2[0])) * (float(bbox2[3]) - float(bbox2[1]))
        ...
        while shared_data.running.value:
            ...
            for result in raw_results:
                ...
                if predictions and len(predictions) > 0:
                    for instance_list in predictions:
