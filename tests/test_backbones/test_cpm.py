...
def test_cpm_backbone():
    with pytest.raises(AssertionError):
        ...
        CPM(in_channels=3, out_channels=17, num_stacks=-1)
    ...
    # Test CPM
    model = CPM(in_channels=3, out_channels=17, num_stacks=1)
    model.init_weights()
    model.train()
    ...
    model = CPM(in_channels=3, out_channels=17, num_stacks=2)
    model.init_weights()
    model.train()
