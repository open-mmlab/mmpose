...
在主流的 COCO 人体关键点数据集上展示了MMPose中主要模型的复杂度信息和推理速度的结果，包括模型的计算复杂度、参数数量，以及以不同的批处理大小在 CPU 和 GPU 上的推理速度。

<details open>
<summary>(点击收起)</summary>

| 算法 | 模型 | 配置文件 | 输入分辨率 | 全类别平均正确率 | 浮点运算次数<br>(10亿次) | 参数数量<br>(百万) | GPU 上的推理速度<br>(每秒处理的帧数)| GPU 上的推理速度<br>(每秒处理的帧数, 批处理大小为10) | CPU 上的推理速度<br>(每秒处理的帧数) | CPU 上的推理速度<br>(每秒处理的帧数, 批处理大小为10) |
| :--- | :---------------: | :-----------------: |:--------------------: | :----------------------------: | :-----------------: | :---------------: |:--------------------: | :----------------------------: | :-----------------: | :-----------------: |
